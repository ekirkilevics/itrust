...
public class ViewMyRemoteMonitoringListActionTest extends TestCase {
	...
	/*public void testIllegalGetPatientDataByDate2() throws Exception {
		DateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy");
        java.util.Date date = new java.util.Date();
        String currentDate = dateFormat.format(date);
		try{
			List<RemoteMonitoringDataBean> data = action.getPatientDataByDate(1L, "2009/01/01", currentDate);
			fail("Format of end date is YYYY/MM/DD, Illegal!");
			//Here to remove warning about data not being read
			data.get(0).getPatientMID();
		}catch(Exception e){
