...
if ("testingForm".equals(submittedFormName)) {
	...
	try {
		...
		action.submitResults(request.getParameter("pidTest"), request.getParameter("numericalResult"), request.getParameter("upperBound"), request.getParameter("lowerBound"));
		...
		response.sendRedirect(request.getContextPath()+"/auth/lt/allProcedures.jsp?action=error&numericalResult=" + request.getParameter("numericalResult") + "&upperBound=" + request.getParameter("upperBound") + "&lowerBound=" + request.getParameter("lowerBound"));
	}
}
